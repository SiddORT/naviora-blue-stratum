...
import { runtimeService, type RuntimeSession } from "@/services/runtime.service";
...
const STATUSES = ["", "Pending", "Launching", "Running", "Completed", "Failed", "Cancelled", "Timed Out"];

export function RuntimeSessionsView() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await runtimeService.listSessions(params);
      if (res.success) {
  ...
  const btnStyle = (active: boolean) => ({
    padding: "4px 10px", borderRadius: 4, border: "1px solid #1E2430", fontSize: 12,
    cursor: "pointer", background: active ? "#D4A63A" : "#141821", color: active ? "#0B0B0F" : "#9CA3AF",
  });
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={s.uuid} style={{ borderBottom: "1px solid #1E2430" }}>
                ...
                <td style={{ padding: "12px 16px" }}>
                  <button onClick={() => router.push(`/admin/runtime/sessions/${s.uuid}`)}
      ...
      {totalPages > 1 && (
        <div style={{ display: "flex", alignItems: "center", gap: 8, justifyContent: "flex-end" }}>
          ...
          <button disabled={page <= 1} onClick={() => setPage(p => p - 1)} style={btnStyle(false)}>Prev</button>
          <span style={{ fontSize: 13, color: "#D1D5DB" }}>{page} / {totalPages}</span>
          <button disabled={page >= totalPages} onClick={() => setPage(p => p + 1)} style={btnStyle(false)}>Next</button>
        </div>
      )}
    </div>
  );

...
interface Props { open: boolean; onClose: () => void; item: WeatherCondition | null; onSuccess: () => void; }

export function WeatherFormDrawer({ open, onClose, item, onSuccess }: Props) {
  const isEdit = !!item;
  ...
  useEffect(() => {
    if (open) {
      reset(item ? {
        name: item.name, wind_speed: item.wind_speed ?? undefined,
        precipitation_level: item.precipitation_level ?? "",
        visibility_range: item.visibility_range ?? undefined,
  ...
  const mutation = useMutation({
    mutationFn: (data: FormData) => isEdit ? weatherService.update(item!.uuid, data) : weatherService.create(data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["master-data-weather"] });
      toast({ variant: "success", title: isEdit ? "Weather condition updated" : "Weather condition created" });
      onSuccess(); onClose();
    },
    ...
    <div className={cn("fixed inset-0 z-50 flex justify-end", !open && "pointer-events-none")}>
      ...
      <div className={cn(
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-card">
          <h2 className="text-base font-semibold text-foreground">{isEdit ? "Edit Weather Condition" : "Add Weather Condition"}</h2>

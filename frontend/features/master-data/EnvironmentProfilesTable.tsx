...
import { envProfileService } from "@/services/master-data.service";
import { toast } from "@/hooks/use-toast";
import { ConfirmModal } from "@/components/ui/ConfirmModal";
import { EnvironmentProfileFormDrawer } from "./EnvironmentProfileFormDrawer";
...
export function EnvironmentProfilesTable() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["master-data-env-profiles"] }); setDeleteTarget(null); toast({ variant: "success", title: "Profile deleted" }); },
  ...
  const toggleMutation = useMutation({
    mutationFn: ({ uuid, active }: { uuid: string; active: boolean }) =>
      active ? envProfileService.deactivate(uuid) : envProfileService.activate(uuid),
  ...
  return (
    <div className="space-y-4">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-muted/40 border-b border-border">
                {["Profile Name","Weather","Sea State","Visibility","Time of Day","Status","Updated","Actions"].map(h => (
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan={8} className="px-4 py-12 text-center text-muted-foreground text-sm">No environment profiles found.</td></tr>
              ) : items.map(ep => (
                <tr key={ep.uuid} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className={cn("inline-flex px-2 py-0.5 rounded-full text-xs font-medium", statusColors[ep.status] ?? statusColors.inactive)}>{ep.status}</span>
                  </td>
                  <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">{formatDate(ep.updated_at)}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      <button onClick={() => { setEditItem(ep); setDrawerOpen(true); }} className="p-1.5 rounded hover:bg-muted text-muted-foreground hover:text-foreground transition-colors" title="Edit"><Pencil className="w-3.5 h-3.5" /></button>
                      <button onClick={() => toggleMutation.mutate({ uuid: ep.uuid, active: ep.is_active })} className="p-1.5 rounded hover:bg-muted text-muted-foreground hover:text-foreground transition-colors" title={ep.is_active ? "Deactivate" : "Activate"}>
                        {ep.is_active ? <PowerOff className="w-3.5 h-3.5" /> : <Power className="w-3.5 h-3.5" />}
                      </button>
      ...
      <EnvironmentProfileFormDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} item={editItem} onSuccess={() => qc.invalidateQueries({ queryKey: ["master-data-env-profiles"] })} />

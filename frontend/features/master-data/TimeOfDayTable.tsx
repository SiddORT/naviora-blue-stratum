...
export function TimeOfDayTable() {
  ...
  const saveMutation = useMutation({
    mutationFn: (data: FormData) => isEdit ? timeOfDayService.update(editItem!.uuid, data) : timeOfDayService.create(data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["master-data-time-of-day"] }); toast({ variant: "success", title: isEdit ? "Updated" : "Created" }); closeForm(); },
    onError: (e: Error) => toast({ variant: "destructive", title: "Error", description: e.message }),
  });
  const deleteMutation = useMutation({
    mutationFn: (uuid: string) => timeOfDayService.delete(uuid),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["master-data-time-of-day"] }); setDeleteTarget(null); toast({ variant: "success", title: "Deleted" }); },
    onError: (e: Error) => { setDeleteTarget(null); toast({ variant: "destructive", title: "Delete failed", description: e.message }); },
  });
  const toggleMutation = useMutation({
    mutationFn: ({ uuid, active }: { uuid: string; active: boolean }) => active ? timeOfDayService.deactivate(uuid) : timeOfDayService.activate(uuid),
  ...
  function openEdit(t: TimeOfDay) { setEditItem(t); setFormOpen(true); }
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <input type="text" placeholder="Search time of day..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} className={cn(filterInput, "pl-9 w-full")} />
        </div>
      ...
      {formOpen && (
        <div className="rounded-xl border border-border bg-card shadow-sm">
          <div className="flex items-center justify-between px-6 py-4 border-b border-border">
            <h3 className="text-base font-semibold text-foreground">{isEdit ? "Edit Time of Day" : "Add Time of Day"}</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="sm:col-span-2">
                <Field label="Name" required error={errors.name?.message}>
                  <input {...register("name")} placeholder="e.g. Dawn" className={inputClass} />
                </Field>
              </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-muted/40 border-b border-border">
                {["Sr No","Name","Description","Status","Updated","Actions"].map(h => (
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan={6} className="px-4 py-12 text-center text-muted-foreground text-sm">No entries found.</td></tr>
              ) : items.map((t, idx) => (
                <tr key={t.uuid} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-4 py-3"><span className={cn("inline-flex px-2 py-0.5 rounded-full text-xs font-medium", statusColors[t.status] ?? statusColors.inactive)}>{t.status}</span></td>
                  <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">{formatDate(t.updated_at)}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      <button onClick={() => openEdit(t)} className="p-1.5 rounded hover:bg-muted text-muted-foreground hover:text-foreground transition-colors" title="Edit"><Pencil className="w-3.5 h-3.5" /></button>
                      <button onClick={() => toggleMutation.mutate({ uuid: t.uuid, active: t.is_active })} className="p-1.5 rounded hover:bg-muted text-muted-foreground hover:text-foreground transition-colors" title={t.is_active ? "Deactivate" : "Activate"}>{t.is_active ? <PowerOff className="w-3.5 h-3.5" /> : <Power className="w-3.5 h-3.5" />}</button>

...
interface Props { open: boolean; onClose: () => void; item: TimeOfDay | null; onSuccess: () => void; }

export function TimeOfDayFormDrawer({ open, onClose, item, onSuccess }: Props) {
  const isEdit = !!item;
  ...
  const mutation = useMutation({
    mutationFn: (data: FormData) => isEdit ? timeOfDayService.update(item!.uuid, data) : timeOfDayService.create(data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["master-data-time-of-day"] });
      toast({ variant: "success", title: isEdit ? "Time of day updated" : "Time of day created" });
      onSuccess(); onClose();
    },
    ...
    <div className={cn("fixed inset-0 z-50", !open && "pointer-events-none")}>
      ...
      <div className={cn(
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-card">
          <h2 className="text-base font-semibold text-foreground">{isEdit ? "Edit Time of Day" : "Add Time of Day"}</h2>

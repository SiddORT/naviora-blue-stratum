...
import { VisibilityFormDrawer } from "./VisibilityFormDrawer";
...
export function VisibilityTable() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["master-data-visibility"] }); setDeleteTarget(null); toast({ variant: "success", title: "Visibility condition deleted" }); },
  ...
  const toggleMutation = useMutation({
    mutationFn: ({ uuid, active }: { uuid: string; active: boolean }) =>
      active ? visibilityService.deactivate(uuid) : visibilityService.activate(uuid),
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <input type="text" placeholder="Search visibility conditions..." value={search}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-muted/40 border-b border-border">
                {["Name","Visibility Distance (NM)","Description","Status","Updated","Actions"].map(h => (
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan={6} className="px-4 py-12 text-center text-muted-foreground text-sm">No visibility conditions found.</td></tr>
              ) : items.map(v => (
                <tr key={v.uuid} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      <button onClick={() => { setEditItem(v); setDrawerOpen(true); }} className="p-1.5 rounded hover:bg-muted text-muted-foreground hover:text-foreground transition-colors" title="Edit"><Pencil className="w-3.5 h-3.5" /></button>
      ...
      <VisibilityFormDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} item={editItem} onSuccess={() => qc.invalidateQueries({ queryKey: ["master-data-visibility"] })} />

...
interface Props { open: boolean; onClose: () => void; item: SeaState | null; onSuccess: () => void; }

export function SeaStateFormDrawer({ open, onClose, item, onSuccess }: Props) {
  const isEdit = !!item;
  ...
  useEffect(() => {
    if (open) {
      reset(item ? {
        name: item.name, wave_height_min: item.wave_height_min ?? undefined,
        wave_height_max: item.wave_height_max ?? undefined,
  ...
  const mutation = useMutation({
    mutationFn: (data: FormData) => isEdit ? seaStateService.update(item!.uuid, data) : seaStateService.create(data),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["master-data-sea-states"] });
      toast({ variant: "success", title: isEdit ? "Sea state updated" : "Sea state created" });
      onSuccess(); onClose();
    },
